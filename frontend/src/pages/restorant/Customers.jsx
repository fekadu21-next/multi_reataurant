...
export default function Customers() {
  ...
  const formatDate = (dateString) => {
    if (!dateString) return t("noOrdersYet");
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-8 animate-in fade-in duration-500 text-slate-900 dark:text-slate-100">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 transition-colors duration-300">
        <div>
          <h2 className="text-3xl font-bold tracking-tight flex items-center gap-3 text-gray-900 dark:text-white">
            <FiUsers className="text-indigo-600 dark:text-indigo-400" /> {t("customerInsights")}
          </h2>
          <p className="text-gray-500 dark:text-slate-400 mt-1">{t("monitorCustomers")}.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="bg-indigo-50 dark:bg-indigo-900/30 px-4 py-2 rounded-lg border border-indigo-100 dark:border-indigo-800/50">
            ...
            <span className="text-indigo-600 dark:text-indigo-400 text-sm ml-2 font-medium">{t("totalCustomers")}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-6 px-4 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-slate-500">
        <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-green-500"></span> {t("active")} (&lt;40d)</div>
        <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-yellow-500"></span> {t("active")} (40d+)</div>
        <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-red-500"></span> {t("inactive")} (90d+)</div>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 space-y-4">
          ...
          <p className="text-gray-400 dark:text-slate-500 font-medium">{t("analyzingCustomers")}...</p>
        </div>
      ) : customers.length === 0 ? (
        <div className="text-center py-20 bg-gray-50 dark:bg-slate-900/50 rounded-2xl border-2 border-dashed border-gray-200 dark:border-slate-800">
          ...
          <h3 className="text-lg font-medium text-gray-900 dark:text-slate-300">{t("noCustomers")}</h3>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4">
          ...
          <div className="hidden md:grid grid-cols-12 px-8 py-3 text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-slate-500">
            <div className="col-span-5">{t("customerInfo")}</div>
            <div className="col-span-2 text-center">{t("orders")}</div>
            <div className="col-span-2 text-center"> {t("totalSpent")}</div>
            <div className="col-span-3 text-right"> {t("totalSpent")}</div>
          </div>
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-12 items-center gap-6">
                ...
                <div className="md:col-span-2 flex flex-col items-center">
                  ...
                  <div className="text-[10px] uppercase font-bold text-gray-400 dark:text-slate-600 mt-1 tracking-widest">{t("orders")}</div>
                </div>
                ...
                <div className="md:col-span-2 flex flex-col items-center">
                  ...
                  <div className="text-[10px] uppercase font-bold text-gray-400 dark:text-slate-600 mt-1 tracking-widest">{t("totalSpent")}</div>
                </div>
