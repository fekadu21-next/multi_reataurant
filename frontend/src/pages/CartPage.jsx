...
export default function CartPage() {
  ...
  return (
    ...
    <div className="w-full mx-auto px-6 md:px-16 lg:px-24 xl:px-32 py-10 bg-white dark:bg-slate-950 text-black dark:text-white transition-colors duration-500">

      {
        cart.map((item) => (
          <div
            key={item.menuItemId}
            className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-[400px_1fr] gap-10 xl:gap-20 bg-white dark:bg-slate-900 p-8 rounded-xl shadow mb-10 border border-gray-100 dark:border-slate-800 max-w-7xl mx-auto"
          >
            {/* IMAGE SECTION */}
            <div>
              <img
                src={item.image}
                alt={item.name}
                className="w-full h-80 xl:h-[450px] object-contain"
              />
      ...
      {/* TOTAL */}
      <div className="max-w-7xl mx-auto text-right text-3xl font-black mt-10 text-black dark:text-white">
